...
def commandline_args_setup():
    ...
    parser.add_argument(commandline_args_token_list[0], help="required input directory path name string for accessing the set of input csv files. Type=string, case sensitive.",
                        type=str)
    parser.add_argument(commandline_args_token_list[1], help="required integer/number of tickers to be writen per output csv file line before a newline is written. Type=integer,{1-200}.",
                        type=int)
    ...
    if commandline_args.arg_str_inputdirpath:
        ...
        print ('Error, exiting. Specify the required command line entry =', commandline_args_token_list[0], ' to specify the input directory path name for accessing the set of input csv files. Type=string, case sensitive')
        exit(1)
    ...
    if commandline_args.arg_int_writenumberoftickersperrow:
        ...
        print ('Error, exiting. Specify the required command line entry =', commandline_args_token_list[1], ' to specify the number of tickers to be writen per output csv file line before a newline is written. Type=integer.')
        exit(1)
    ...
    if commandline_args.arg_str_output_enable_filename_datestamping:
        ...
        print ('Error, exiting. Specify the required command line entry =', commandline_args_token_list[2], ' to specify if the output filename should include a current date YYYYMMDD timestamp. Type=string,{True/False},case insensitive.')
        exit(1)
...
def input_chdir_geninput_csvfilename_list():
    try:
        ...
        print ('Error, exiting: an os.chdir exception occurred when changing to directory = ({0}); os.chdir exception = {1}'.format(commandline_args.arg_str_inputdirpath, exc))
        raise
        exit(1)

    input_csvfilename_list = glob.glob('*.csv')
    if (len(input_csvfilename_list) == 0):
        print ('Error, exiting: the specified input directory did not include any .csv files. Input directory = {0}'.format(commandline_args.arg_str_inputdirpath))
        exit (1)
...
def input_readallrows():
    ...
    print ('Step 1, input... reading input filename = {0}'.format(commandline_args.arg_str_inputdirpath + '//' + input_filename))
...
def input_parsetickers():
    ...
    if (len(ticker_list) == 0):
        print ('Error, the specified input file did not include any stock symbol tickers. Input directory and filename = {0}'.format(commandline_args.arg_str_inputdirpath + input_filename))
    else:
...
def output_filename_setup():
    ...
    if commandline_args.arg_str_output_enable_filename_datestamping.lower() == 'true': # set per input argument
        ...
        output_filename_regex_split_pattern = re.compile(r'(.*)(20)([0-9]{6})(.*)', re.IGNORECASE)  # specify preceding 'r' for raw to avoid need to precede w/ backslashes; Specify '?' for non-greedy to get 1st occur only
        output_filename_text_match = output_filename_regex_split_pattern.search(output_filename)              # search returns a match object or none
        if (output_filename_text_match):
            if 0:
                ...
                print('output_filename_text_match.group(2) = ' + output_filename_text_match.group(2)) # Filename date part 1 as '20'
                print('output_filename_text_match.group(3) = ' + output_filename_text_match.group(3)) # Filename date part 2 as 'YYMMDD
